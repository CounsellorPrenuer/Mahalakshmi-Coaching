...
function validProjectIds(env: Env) {
  const fromEnv = env.VALID_PROJECT_IDS.split(",").map((item) => item.trim()).filter(Boolean);
  return new Set([...fromEnv, ...CMS_CONFIGURED]);
}

function validProject(projectId: string, env: Env) {
  return validProjectIds(env).has(projectId);
}
